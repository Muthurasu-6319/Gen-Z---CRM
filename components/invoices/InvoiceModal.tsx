    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (invoiceToEdit) {
                ...
                setClientDetails({
                    username: client?.username || invoiceToEdit.client_name_override || '',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (invoiceToEdit && !isDuplicate) {
                ...
                invoiceData.notes = invoiceToEdit.notes;
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={invoiceToEdit ? (isDuplicate ? "Duplicate Invoice" : "Edit Invoice") : "Create New Invoice"}>
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                <InputField label="Client Name" type="text" value={clientDetails.username} onChange={handleClientNameChange} list="client-list" placeholder="Type or select an existing client" required />
                <datalist id="client-list">
                    {clients.map(client => <option key={client.id} value={client.username} />)}
                </datalist>
